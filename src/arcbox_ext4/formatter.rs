...
pub struct FormatOptions {
    ...
    /// vmbuild patch 4: timestamp `(seconds_lo, extra)` for the inodes the
    /// formatter creates on its own -- the root directory and `/lost+found`.
    ///
    /// Upstream read the wall clock for both, so two runs over identical
    /// input produced different bytes. With this set (and an explicit `uuid`,
    /// and explicit per-entry timestamps) the image is byte-reproducible,
    /// which turns "did we build the same thing?" into a hash comparison.
    ///
    /// `None` keeps upstream behaviour.
    pub epoch: Option<(u32, u32)>,
    /// vmbuild patch 5: allow the final block group to be partial.
    ///
    /// Upstream always rounded the image up to a whole block group
    /// (32768 blocks x 4096 = 128 MiB), so a 129 MiB request produced a
    /// 256 MiB file. `mke2fs` permits a short final group, and the extra
    /// apparent bytes are not free downstream: heyvm's per-VM copy is dense
    /// on ext4 hosts, and `heyvm mvm build` streams the image to S3 where
    /// holes upload as zeros.
    ///
    /// `false` keeps upstream behaviour.
    pub allow_partial_final_group: bool,
}
...
impl FormatOptions {
    ...
    /// vmbuild patch 4: fix the timestamp used for the root inode and
    /// `/lost+found`.
    pub fn epoch(mut self, lo: u32, hi: u32) -> Self {
        self.epoch = Some((lo, hi));
        self
    }
...
pub struct Formatter {
    ...
    /// on construction.
    label: Option<String>,
    /// vmbuild patch 5: let the final block group be short instead of rounding
    /// the image up to a whole 128 MiB group.
    allow_partial_final_group: bool,
}
...
impl Formatter {
    ...
    pub fn with_options(path: &Path, opts: FormatOptions) -> FormatResult<Self> {
        ...
        let mut root = Inode::root_inode(); // inode 2 -- root directory
        // vmbuild patch 4: root_inode() stamps the wall clock; override it so
        // the image can be byte-reproducible.
        if let Some((lo, hi)) = opts.epoch {
            root.atime = lo;
            root.ctime = lo;
            root.mtime = lo;
            root.crtime = lo;
        ...
        let mut fmt = Self {
            ...
            uuid: opts.uuid,
            label: opts.label,
            allow_partial_final_group: opts.allow_partial_final_group,
        };
        ...
        // vmbuild patch 4: pass an explicit timestamp so /lost+found does not
        // fall back to FileTimestamps::default(), which reads the clock.
        let lf_ts = opts.epoch.map(|(lo, hi)| FileTimestamps {
            access_lo: lo,
            access_hi: hi,
            modification_lo: lo,
            modification_hi: hi,
    ...
    pub fn create(
        ...
        if is_dir(mode) {
            ...
            if link_bytes.len() < 60 {
                // vmbuild patch 2: an inline symlink consumes no data blocks,
                // but do NOT zero blocks_lo -- an xattr block may already have
                // been charged above. It is 0 here unless that happened.
            } else {
    ...
    pub fn close(mut self) -> FormatResult<()> {
        ...
        //
        // vmbuild patch 5 (second site): upstream rounded the image up to
        // `min_groups` *whole* groups as soon as the written data spilled past
        // a group boundary -- so a 212 MiB request holding 153 MiB of data
        // became 256 MiB. When partial final groups are allowed, only
        // guarantee the image covers what was actually written: `data_size`
        // already counts the data blocks plus both bitmap blocks per group.
        if self.allow_partial_final_group {
            let needed = data_size as u64 * self.block_size as u64;
            if self.size < needed {
                self.size = needed;
        ...
        // Align disk size to a block-group boundary.
        //
        // vmbuild patch 5: skipped when partial final groups are allowed, so
        // a 129 MiB request stays 129 MiB instead of becoming 256 MiB. The
        // short group's unusable tail is marked allocated in its block bitmap
        // below, and `blocks_count` is taken from the real size, so e2fsck
        ...
        // vmbuild patch 5: a partial final group still has to hold its own
        // metadata -- two bitmap blocks plus a full inode table. A tail
        // shorter than that would describe an inode table running past the
        // end of the filesystem, which e2fsck rejects. Round such a tail up
        // to the minimum viable size (a couple of MiB) rather than up to a
        // whole 128 MiB group.
        if self.allow_partial_final_group {
            // Whole blocks only. Without this the image keeps whatever byte
            // count the caller's heuristic produced, which need not be a
            // multiple of the block size -- or even of 512. These files are
            // attached to VMs as block devices, so a ragged tail is asking
            // for trouble.
            let bs = self.block_size as u64;
            self.size = self.size.div_ceil(bs) * bs;

            let bpg = self.blocks_per_group() as u64;
            let fs_blocks = self.size / bs;
            let rem = fs_blocks % bpg;
            let min_tail = (inode_table_size_per_group + 2) as u64;
            if rem != 0 && rem < min_tail {
                self.size = (fs_blocks - rem + min_tail) * bs;
            }
        }

        // The file must still physically span the groups we describe.
        ...
        for group in 0..block_groups {
            ...
            // vmbuild patch 5: when this is the last group and the filesystem
            // ends partway through it, the blocks past the end do not exist.
            // Set their bitmap bits so nothing tries to allocate them --
            // ext4's padding convention -- but deliberately do NOT count them
            // as used. They are not blocks; counting them made
            // `total_used_blocks` exceed the real block count, which drove the
            // superblock's free-block count to a u32 underflow (e2fsck:
            // "the filesystem size according to the superblock is N blocks,
            // the physical size of the device is M blocks").
            //
            // `group_real_blocks` is what this group actually contains, and is
            // what its free count is measured against.
            let mut group_real_blocks = self.blocks_per_group();
            if self.allow_partial_final_group && group == total_groups_u32 - 1 {
                let fs_blocks = (self.size / self.block_size as u64) as u32;
                group_real_blocks = fs_blocks
                    .saturating_sub(group_start)
                    .min(self.blocks_per_group());
                for i in group_real_blocks..self.blocks_per_group() {
                    bitmap[(i / 8) as usize] |= 1 << (i % 8);
                }
            }
            ...
            // vmbuild patch 5: measured against the blocks this group really
            // has, which is short of a full group for a partial final group.
            let free_blocks = group_real_blocks.saturating_sub(used_blocks);
        ...
        // vmbuild patch 5: with a partial final group the filesystem is
        // shorter than groups x blocks_per_group, and the superblock must say
        // so -- otherwise it claims blocks past the end of the file.
        let mut blocks_count = if self.allow_partial_final_group {
        ...
        // vmbuild patch 1: upstream wrote 3 (FreeBSD). These images are consumed
        // by Linux guests and by host-side e2fsprogs (e2fsck/resize2fs/tune2fs),
        // and creator_os changes how several inode fields are interpreted --
        // notably i_blocks, i_frag/i_fsize, and the high uid/gid halves.
