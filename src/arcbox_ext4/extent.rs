...
fn write_inline_extents(
    ...
    // vmbuild patch 2: accumulate rather than assign. The caller may already
    // have charged this inode for a separate xattr block (formatter.rs), and an
    // assignment here silently discarded it, leaving i_blocks short by one block
    // and e2fsck reporting the inode. `blocks_lo` starts at 0 and write_extents
    // runs at most once per inode, so `+=` is equivalent everywhere else.
...
fn write_indexed_extents<W: Write + Seek>(
    ...
    // vmbuild patch 2: accumulate rather than assign -- see write_extents above.
